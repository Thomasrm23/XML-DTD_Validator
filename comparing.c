...
// debut test
    //printf("%d\n\n", doc_xml->child_number);
   // printf("%d\n", classrooms_xml->children.size);

    XMLNode *test_first = XMLNode_child(XMLNode_child(doc_xml->root, 0), 0);
    //printf("nombre d'attributs du 1er enfant :%d\n", test_first->attributes.size);


    for(int k=0; k<test_first->attributes.size; k++){
        DTDNode *attr_dtd = DTDNode_child(doc_dtd->root, k+2);
    ...
    //fin test
        ...
        for(int i=0; i<classrooms_xml->children.size; i++){
            ...
            /*printf("\n--------------taille test-------------%d", test_first->attributes.size);
            printf("\n--------------taille current-------------%d", childs->attributes.size);
            */
            if (test_first->attributes.size != childs->attributes.size){
                printf("Dans votre xml des elements n'ont pas le meme nombre d'attributs");
                return 0;
            }



                XMLAttribute attr = childs->attributes.data[k];

                if (attr.key == 0){
                    printf("Il manque un attribut a votre balise \"%s\" numero %d", childs->tag, i+1);
                    return 0;
                }
/*
